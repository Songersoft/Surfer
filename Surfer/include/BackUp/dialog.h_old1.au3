...
func gfxbinrotate()
	...
	if $quit= 0 then
		...
		$win2= guicreate("Create rotated frames", 420, 475, $loc[0]+320, $loc[1]+200, default, default, $hgui)
		$controls= 0
		...
		$help= "Select a source to save rotated frames to a gfxbin"&@crlf
		$help&= "You can position and resize the bounding box of the rotated frames"

		guictrlsetdata($control[59][0], $help)
		guisetstate()
		...
		if $confirm= 1 then
			...
			if $confirm= 1 then
				...
				$gfxbindata[$gfxbins].anglemake(0, $source[$scur], $dsurf, $px, $py, $qx, $qy, $rotanginc, $astart, $aend, $binstart, $source[$scur].colorkey, )
				...
				if $rottype= 0 then
					...
					if $root= -1 then
						$gfx[$gfxs].make($gfxbins-1, $tname, $root, 0, 0, 0, 15, 15, $source[$scur].fromsource, $tname, $qx, $qy)
					else
						$gfx[$gfxs].make($gfxbins-1, $tname, $root, 0, 0, 0, _
							$source[$scur].fromx-$dx/2+1, $source[$scur].fromy-$dy/2+1, $source[$scur].fromsource, $tname, $qx, $qy)
					endif
				else
					if $root= -1 then
						$gfx[$gfxs].make($gfxbins-1, $tname, $root, 0, 0, 0, 15, 15, $source[$scur].fromsource)
					else
						$gfx[$gfxs].make($gfxbins-1, $tname, $root, 0, 0, 0, _
							$source[$scur].fromx-$dsurfw/2+$source[$scur].filew, $source[$scur].fromy-$dsurfh/2, $source[$scur].fromsource, -1, $qx, $qy)
					endif
				endif;endif rottype
			endif;endif $confirm yes two
...
func usernamebin()
	local $controls= 0, $r= 0, $no= 1
	$win4= guicreate("Name this object", 320, 200, $loc[0]+200, $loc[1]+200, default, default, $hgui)
	$control= loaddialogquick(@scriptdir&"\system\dialogs\gfxadd.txt", $controls)
